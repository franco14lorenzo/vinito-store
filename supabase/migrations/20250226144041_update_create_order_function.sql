...
BEGIN
  ...
  FOR item IN SELECT * FROM jsonb_to_recordset(items) AS (
    ...
    LOOP
      ...
      -- Check for insufficient reserved stock
      IF (SELECT reserved_stock FROM wines WHERE wines.id = wine.wine_id) < 0 THEN
        RAISE EXCEPTION 'Insufficient reserved stock for wine %', wine.wine_id;
      END IF;
